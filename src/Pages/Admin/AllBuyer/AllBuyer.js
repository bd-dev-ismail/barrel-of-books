  ...
  const closeModal = () => {
    return setRemoveBuyer(null);
     ...
     const { data: buyers, isLoading , refetch} = useQuery({
       queryKey: ["buyer"],
       queryFn: async () => {
         const res = await fetch(`https://barrel-of-books-server.vercel.app/buyer`, {
           headers: {
     ...
     const handalDelete = (buyer) => {
       fetch(`https://barrel-of-books-server.vercel.app/buyer/${buyer?._id}`, {
         method: "DELETE",
         headers: {
           authorization: `Bearer ${localStorage.getItem("accessToken")}`,
         },
    ...
    return (
      <div>
        <Helmet>
          <title>All Buyers -Barrel Of Books</title>
        </Helmet>
